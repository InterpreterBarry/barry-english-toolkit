...
> 万能寒暄和反思框架:**咋了 → 那咋了 → 然后呢**。三段论,既可以用来跟人聊天,也可以用来自己复盘。
...
- **What**:发生了什么事(事实层)
- **So What**:这事儿为什么重要(意义层)
- **Now What**:接下来怎么办 / 抛给对方(行动层)

**一句话记忆**:咋了 → 那咋了 → 然后呢。

## WHY:为什么这个框架值得专门记

**因为它解决的是"没话聊"的尴尬**——社交场合、职场寒暄、会后闲聊,大脑一片空白时,这三步可以直接填满。

但它也有天花板:**太 general**。如果你只会这一个框架,没有其他内容储备,会出现"虽然在按框架走,但没实质内容"的空转。

所以 Barry 的定位很明确:**它是寒暄专用,不适合做干货输出**。不需要输出什么高深观点的场景 → 它几乎万能;要讲干货的场景 → 用 [[Problem-Solution-Benefit]] 或 [[ADD-问答模型]]。
