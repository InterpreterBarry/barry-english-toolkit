...
启动时一次性 `Read ~/.barry-english/methodology/wiki/_index.json`,这是 wiki 页的 `ID → 相对路径` 映射表(相对 `~/.barry-english/methodology/wiki/`)。本 skill 后续遇到 `ID:xxx` 标记,从 index 查路径,再 Read 对应 wiki 文件。这样以后 wiki 文件改名/移动时只需要重跑 `build-wiki-index.py`,本 skill 不需要改。

**_index.json 或 wiki 目录不存在时的处理**:
- 不要 abort,**给学员明确指引**:
  > "我找不到 methodology wiki(本来应该装在 `~/.barry-english/methodology/`)。这是工具箱安装时漏装了。修复:
  > ① 找到你 toolkit 仓库 clone 的位置(如 `~/Desktop/Barry-English-Toolkit/`)
  > ② 重跑 `bash <toolkit-root>/scripts/install.sh`(会重装 skill + methodology + _index.json)
  > 或者重跑 `/barry-onboarding`,我会带你重装一遍。"
...
| 方法论入口 / 整体框架 / 你这套是什么 | `~/.barry-english/methodology/index.md`(顶层目录,不在 _index.json 里) | `ID:mindset-overview` |
...
4. **不暴露 wiki 路径给学员**(学员不需要知道 `~/.barry-english/methodology/wiki/02-翻译能力/MCES模型.md`)
...
**Barry 风格回答**(读 `~/.barry-english/methodology/wiki/02-翻译能力/MCES模型.md` 后):
...
- ❌ **不暴露 wiki 路径**(`~/.barry-english/methodology/wiki/02-翻译能力/MCES模型.md` 这种是 coach 的内部路由,不给学员看)
...
| 不存"今日问答记录"持久化 | Claude Code session 本身就是日志,关掉就关掉 |
...
学员说"你刚才那个 MCES 是从哪里读的?" → 不直接报内部绝对路径,简短说:"是 Barry 方法论 wiki 里的核心概念页,wiki 跟着工具箱在你电脑上 `~/.barry-english/methodology/`,可以自己翻。"
...
barry-coach ⭐       任何时候并行调用(Q&A)
                    ↓
                    读 ~/.barry-english/methodology/wiki/ 答方法论问题
...
- **[2026-05-14]** 新建 v0.1,RAG 模式(读 `~/.barry-english/methodology/wiki/` 原文)+ 4 类场景路由 + Barry 风格规范 + 主题地图(query → wiki 板块)+ 失败模式预防
