...
>
> 装好之后:
> - 每天:**新开一个 Claude Code session,输入"每日训练"** 启动专属训练
...
> - **专属每日训练 Skill** `~/.claude/skills/barry-daily-<你的昵称>/`
...
> **每天怎么用**:
>
> 1. **训练**:新开一个 Claude Code session(或直接在当前 session)→ 输入 "**每日训练**" → 跑完你的当日训练菜单
...
> - 专属训练 Skill:`~/.claude/skills/barry-daily-<昵称>/`
