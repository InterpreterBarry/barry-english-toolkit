---
name: barry-solution
description: Barry 学员训练方案设计 + 自动生成专属每日训练 Skill。读学员档案 Section 1+2+3 + Four-Strands 训练方法工具箱,设计 Section 4 训练方案,获学员授权后自动安装到 ~/.claude/skills/barry-daily-<昵称>/,并复制 Quiz 网页代码到 ~/.barry-english/quiz-web/。**前置:必须先跑完 barry-profile + barry-assessment**。10-15 分钟。触发词:定制方案、出方案、训练方案、生成训练 Skill、barry solution。
---

# Barry 学员档案 · Section 4 (Solution) + 自动生成专属训练 Skill

> 10-15 分钟,基于学员档案 Section 1+2+3 + [[Four-Strands-训练方法工具箱]],
> 设计 Section 4 训练方案 + 自动安装 `~/.claude/skills/barry-daily-<昵称>/`。
> 隶属 Barry 职场英语工具箱。
...
写入 `~/.claude/skills/barry-daily-<昵称小写无空格>/SKILL.md`(经学员授权后自动安装)。
...
10. **自动安装需学员授权** —— Step 4 必须先取得明确"同意安装"才能写 `~/.claude/skills/`
...
Step 4 · 生成 + 自动安装专属训练 Skill (3-5 min)
   4A · 取学员授权
   4B · 模板填空生成 SKILL.md
   4C · 自动安装到 ~/.claude/skills/barry-daily-<昵称>/
...
> 接下来我要做两件事:
>
> 1. 在 `~/.claude/skills/barry-daily-<你的昵称>/` 装一个**专属每日训练 Skill**
...
#### 4C · 自动安装到 `~/.claude/skills/barry-daily-<昵称-slug>/`

```
1. 检查路径 ~/.claude/skills/ 是否存在;不存在则 mkdir -p

2. 扫描已有 barry-daily-* 目录(可能是学员之前用别的昵称跑过 onboarding 留下的残留):
   ls -d ~/.claude/skills/barry-daily-* 2>/dev/null
   - 若返回 0 个 → 跳到 3
   ...
   >
   > 旧的怎么处理?
   > A. 全部删除(推荐,避免触发"每日训练"时 Claude Code 不知道用哪个)
...
**Quiz Web**:源码已经预先 sanitize + ship 进 toolkit,作为 `barry-solution` Skill 的子目录(`quiz-web/`)。学员安装 Skill 时跟着一起装到 `~/.claude/skills/barry-solution/quiz-web/`。

**复制命令**(两行):

```bash
mkdir -p ~/.barry-english
cp -R ~/.claude/skills/barry-solution/quiz-web ~/.barry-english/quiz-web
```

> **Methodology wiki 不在这里装** —— 由 `scripts/install.sh` 在工具箱整体安装时已经拷到 `~/.barry-english/methodology/`(含 `_index.json`)。barry-solution 不需要重复拷贝。如果发现 wiki 不在,提示学员重跑 `bash <toolkit-root>/scripts/install.sh`。
...
> 装好了!给你梳理一下接下来怎么用:
>
> **专属训练 Skill** 装在 `~/.claude/skills/barry-daily-<昵称-slug>/SKILL.md`
> 用法:**新开一个 Claude Code session**,直接输入 "**每日训练**"(或 `/barry-daily-<昵称-slug>`)启动。
>
...
>
> **每天**(按你的时间安排):
> 1. **新开一个 Claude Code session** → 输入 "每日训练" → 跑专属训练 Skill 完成今日训练
...
```
~/.claude/skills/barry-daily-<昵称-slug>/
...
- [ ] 学员授权安装(明确同意)
- [ ] 专属训练 Skill 写入 `~/.claude/skills/barry-daily-<昵称-slug>/SKILL.md`
- [ ] `~/.barry-english/training-log.md` 占位文件创建
- [ ] `~/.barry-english/fancy-vocab.md` 占位文件创建
- [ ] `~/.barry-english/quiz-web/` Quiz 代码复制 + sanitize 完成
- [ ] 启动指引已告知学员(新开 Claude Code session + 训练 Skill 触发词 + Quiz Web 启动方式)
- [ ] 提醒学员"情况有变化随时跟 AI 说"
...
| `~/.claude/skills/barry-daily-<昵称-slug>/` 已存在(学员之前装过)| 询问"是否覆盖旧版?"(可能重跑 onboarding 后档案变了);学员同意 → 备份旧 SKILL.md 为 .bak.<timestamp> 再覆写 |
| 学员说"我没有 Claude Code 但想看方案" | 仍写 Section 4 到档案,Step 4 改为"输出 SKILL.md 内容贴出来给你,等以后装 Claude Code 再用";Quiz Web 启动指引也给(node 即可,不依赖 Claude Code)|
...
| 训练 Skill 自动安装路径权限不足 | 告知"我没法自动写 ~/.claude/skills/,可能权限问题"+ 提供完整 SKILL.md 内容让学员手动复制 |
